...
gen_pop_X <- function(N = 20000, 
  ...
  pop_dat <-
    ...
    mutate(x1 = rbinom(N, size = 1, prob = 0.3 + 0.5 * id_area / C),
           ...
           x3 = rbinom(N, size = 1, prob = 0.1 + 0.2 * id_area / C),
           x4 = rbinom(N, size = 1, prob = 0.5 + 0.3 * id_area / C),
           x5 = pmax(rpois(N, lambda = 3 * (1 - 0.1 * id_area / C)), 1),
           x6 = rbinom(N, size = 1, prob = 0.4))
  pop_dat
}

gen_pop_Y <- function(pop_dat, eta_c_sd = .15,
                      e_ch_sd = .5,
                      beta = c(3, .09, -.04, -.09, .4, -.25, .1)) {
  N <- nrow(pop_dat)
  C <- length(unique(pop_dat$id_area))
  pop_X <- model.matrix(~x1 + x2 + x3 + x4 + x5 + x6, pop_dat)
  pop_dat %>%
    mutate(eta_c = rep(rnorm(C, sd = eta_c_sd), each = N / C),
           e_ch = rnorm(N, sd = e_ch_sd),
           y = exp(pop_X %*% beta + eta_c + e_ch))
}
...
run_one_sim <- function(sample_ind, pop_dat_X, pop_gen_fn, i, ...) {
  ...
  working_fit <- svyglm(z ~ x1 + x2 + x3 + x4 + x6, 
                        sample_des, family = quasibinomial())
  greg_est <- get_greg(working_fit, z ~ x1 + x2 + x3 + x4 + x6,
                       ~region, pop_dat, sample_des)
  iid_sgreg_logit_est <- get_iid_sdir_logit(greg_est)
  iid_sgreg_est <- get_iid_sdir(greg_est)
  
  working_full_fit <- svyglm(z ~ x1 + x2 + x3 + x4 + x5 + x6, 
                             sample_des, family = quasibinomial())
  greg_full_est <- get_greg(working_full_fit, z ~ x1 + x2 + x3 + x4 + x5 + x6,
                            ~region, pop_dat, sample_des) %>%
  ...
  iid_y_ulm_est <- get_iid_gau_ulm(log(y) ~ x1 + x2 + x3 + x4 + x6, 
                                   ~region, pop_dat, sample_dat,
                                   h = function(x) exp(x) < 10.2)
  iid_z_ulm_est <- get_iid_bin_ulm(z ~ x1 + x2 + x3 + x4 + x6,
                                   ~region, 
                                   sample_dat = sample_dat,
                                   pop_dat = pop_dat)
  
  iid_y_full_ulm_est <- get_iid_gau_ulm(log(y) ~ x1 + x2 + x3 + x4 + x5 + x6, 
                                        ~region, pop_dat, sample_dat,
                                        h = function(x) exp(x) < 10.2) %>%
    mutate(method = "iidGauFullULM")
  iid_z_full_ulm_est <- get_iid_bin_ulm(z ~ x1 + x2 + x3 + x4 + x5 + x6, 
                                        ~region, 
                                        sample_dat = sample_dat,
                                        pop_dat = pop_dat) %>%
...
# Sys.time() - t
# t <- Sys.time()
# temp2 <- run_one_sim(inf_sample_ind,  pop_dat, gen_pop_Y, 1)
# Sys.time() - t
...
saveRDS(res, paste0(sim_res_dir, "CMN-improved-model_res_", k, ".rds"))
k = as.numeric(args[1])
set.seed(k)
res <- do.call(
  rbind,
...
saveRDS(res, paste0(sim_res_dir, "CMN-improved-model-inf-smp_res_", k, ".rds"))
