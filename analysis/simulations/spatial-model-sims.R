...
gen_pop_Y <- function(pop_dat, eta_a_sd = .15,
                      e_ac_sd = .5,
                      beta = c(3, .4, -.1, .15, .15, .15, .25, .05, .05)) {
  N <- nrow(pop_dat)
  A <- length(unique(pop_dat$admin1_name))
...
run_one_sim <- function(sample_ind, pop_dat_X, pop_gen_fn, i, ...) {
  ...
  working_full_fit <- svyglm(z ~ x1_ns + x1_ns_sig + x1_BYM2 + x2_BYM2 + 
                               x1_SPDE + x2_SPDE + l1a + poverty, 
                             sample_des, family = quasibinomial())
  greg_full_est <- get_greg(working_full_fit, 
                            z ~ x1_ns + x1_ns_sig + x1_BYM2 + x2_BYM2 + 
                              x1_SPDE + x2_SPDE + l1a + poverty,
                            ~region, pop_dat, sample_des) %>%
  ...
  iid_y_ulm_est <-
    get_iid_gau_ulm(
      formula = log(y) ~ x1_ns + x1_ns_sig + 
        x2_BYM2 + x1_SPDE + l1a + poverty, 
      by = ~region, pop_dat = pop_dat, 
      sample_dat = sample_dat, h = function(x) exp(x) < 12)
  ...
  iid_y_full_ulm_est <- 
    get_iid_gau_ulm(
      formula = log(y) ~ x1_ns + x1_ns_sig + x1_BYM2 + 
...
set.seed(1201)
sample_ind_res <- gen_sample_ind(pop_dat, ~stratum, .05)
inf_sample_ind_res <- gen_sample_ind_inf(pop_dat, ~stratum, .05)
...
saveRDS(res, paste0(sim_res_dir, "CMN-spatial-model_res_", k, ".rds"))
k = as.numeric(args[1])
set.seed(k)
res <- do.call(
  rbind,
...
saveRDS(res, paste0(sim_res_dir, "CMN-spatial-model-inf-smp_res_", k, ".rds"))
